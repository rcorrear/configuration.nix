...
(defn temp-root
  ...
  (let [root (fs/create-temp-dir {:prefix "configuration-nix-graphify-test-"})]
    ...
    (spit (str (fs/path root "devenv.lock")) "graphify-lock\n")
...
(deftest graphify-config-hash-includes-graphify-configuration
  (let [root (temp-root)
        before (common/config-hash root "graphify 1.0")]
    (try (spit (str (fs/path root "devenv.lock")) "changed-lock")
