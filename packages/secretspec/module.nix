...
let
  ...
  isTomlBareKey = name: builtins.match "[A-Za-z0-9_-]+" name != null;
  ...
  secretNameAssertions = map (name: {
    assertion = name != "" && !lib.hasInfix "/" name && isTomlBareKey name;
    message = "secretspec: secret `${name}` has an invalid name; secret names must be non-empty TOML bare keys containing only A-Z, a-z, 0-9, `_`, or `-`";
  }) allSecretNames;
  ...
  serviceNameAssertions = lib.flatten (
    lib.mapAttrsToList (
      name: s:
      map (svc: {
        assertion = svc != "" && !lib.hasInfix "/" svc && isTomlBareKey svc;
        message = "secretspec: secret `${name}` references service `${svc}` with an invalid name; service names must be non-empty TOML bare keys containing only A-Z, a-z, 0-9, `_`, or `-`, and must not contain `/` (used to build /run/secrets/<scope>.env paths and scope arguments)";
      }) s.services
    ) allSecrets
  );
in
                  ...
                  options = {
                    ...
                    delivery = lib.mkOption {
                      ...
                      description = ''
                        ...
                        - `envvar`: appears in scope export files (`NAME=value`). Values containing CR or LF are rejected because scope exports are used as systemd EnvironmentFiles. Not written as a standalone file.
          ...
          script = ''
            ...
            ${lib.concatStringsSep "\n" (
              lib.mapAttrsToList (
                ...
                ''
                  (
                    out_path=${lib.escapeShellArg outPath}
                    out_dir="$(dirname -- "$out_path")"
                    mkdir -p "$out_dir"
                    umask 0077
                    tmp_path="$(mktemp "$out_dir/.secretspec.XXXXXX")"
                    trap 'rm -f -- "$tmp_path"' EXIT
                    ${cfg.package}/bin/secretspec get ${lib.escapeShellArg name} > "$tmp_path"
                    chown ${lib.escapeShellArg secret.owner}:${lib.escapeShellArg secret.group} "$tmp_path"
                    chmod ${lib.escapeShellArg secret.mode} "$tmp_path"
              ...
              lib.mapAttrsToList (
                ...
                ''
                  ...
                  (
                    ...
                    tmp_path="$(mktemp "$out_dir/.secretspec.XXXXXX")"
                    trap 'rm -f -- "$tmp_path"' EXIT
                    ${pkgs.gettext}/bin/envsubst '${data.shellFormat}' < ${data.templateFile} > "$tmp_path"
                    chown ${lib.escapeShellArg secret.owner}:${lib.escapeShellArg secret.group} "$tmp_path"
                    chmod ${lib.escapeShellArg secret.mode} "$tmp_path"
                    mv -f -- "$tmp_path" "$out_path"
                    trap - EXIT
            ...
            ${lib.concatStringsSep "\n" (
              lib.mapAttrsToList (
                ...
                let
                  ...
                  outPath = "/run/secrets/${scope}.env";
                  expectedLines = builtins.length _s.secrets;
                in
                ''
                  (
                    out_path=${lib.escapeShellArg outPath}
                    ...
                    tmp_path="$(mktemp "$out_dir/.secretspec.XXXXXX")"
                    trap 'rm -f -- "$tmp_path"' EXIT
                    ${cfg.package}/bin/secretspec export --scope ${lib.escapeShellArg scope} --format shell > "$tmp_path"
                    # SecretSpec emits one assignment line per secret. Extra lines
                    # can only be embedded secret data, which this format rejects.
                    if [ "$(wc -l < "$tmp_path")" -ne ${toString expectedLines} ] || ${pkgs.gnugrep}/bin/grep -q $'\r' "$tmp_path"; then
                      echo "secretspec: scope ${lib.escapeShellArg scope} contains a multiline envvar secret, which cannot be safely written to an EnvironmentFile" >&2
                      exit 1
                    fi
                    ${pkgs.gnused}/bin/sed -i 's/^export //' "$tmp_path"
                    chown ${lib.escapeShellArg owner}:${lib.escapeShellArg group} "$tmp_path"
                    chmod 0400 "$tmp_path"
                    mv -f -- "$tmp_path" "$out_path"
                    trap - EXIT
                  )
