...
1. Conceptually, Den walks the context type's `into.*` transitions recursively,
   building a list of `{ ctx, ctxDef, source }` pairs.
2. It then applies `den.lib.parametric.fixedTo` for the first occurrence of each
   context type and `den.lib.parametric.atLeast` for subsequent ones, preventing
   duplicate owned configs.
3. The result is a flat list of aspect fragments merged into a deferred module.

## Custom Contexts

You can define your own alternative context pipelines outside of `den.ctx.host` or 
