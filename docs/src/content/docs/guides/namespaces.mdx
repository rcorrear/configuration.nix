...
{ inputs, den, ... }: {
  imports = [
    # Option A: Create "my" namespace (not exported to flake outputs)
    (inputs.den.namespace "my" false)

    # Option B: Create and export "eg" namespace
    # (inputs.den.namespace "eg" true)
  ];
}
```

Only one of these options should be used at a time (uncomment the desired option).
