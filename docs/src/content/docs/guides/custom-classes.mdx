...
when `environment.persistence` option is available in the host (the impermanence module was imported in host)
...
The forward-guard itself is responsible for checking in only one place, instead of having `mkIf` in many places.

```nix
# Custom `persys` forwards config into nixos.environment.persistence."/nix/persist/system"
# only if environment.persistence option is present.
...
# becomes nixos.environment.persistence."/nix/persist/system".hideMounts = true;
