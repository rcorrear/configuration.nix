...
- **Keep legacy working.** `import-tree` loads your existing files alongside
  Den aspects. Modules are merged via the Nix module system, so overlapping option
  definitions may surface errors. Resolve conflicts by renaming, overriding, or patching
  conflicting options, or use selective imports to avoid merge errors.
- **Use batteries.** Replace manual user/shell/HM setup with `den.provides.*`.
- **Test with VM.** Build or run the host-specific flake output (e.g., `nix build .#nixosConfigurations.<hostname>.config.system.build` or `nix run .#nixosConfigurations.<hostname>`) to validate changes before applying to hardware.
