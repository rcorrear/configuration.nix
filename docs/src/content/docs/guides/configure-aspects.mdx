...
Den **automatically** creates an aspect for each of your host/user/home definitions. For example, with the following definitions:
...
`(1)` and `(2)` are termed *static aspects* and are the terminal leaves of aspects DAG.
...
`(3)` is a more interesting kind of aspect that is used by Den to pass host/user definitions
