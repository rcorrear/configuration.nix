      ...
      }:
      {
        ...
        systemd.services.renovate = {
          ...
          serviceConfig.LoadCredential = [
            "renovateGithubAppPrivateKey:${config.services.secretspec.secretPaths.renovateGithubAppPrivateKey}"
            "renovate.env:${config.services.secretspec.scopeExportPath.renovate}"
          ];
          script = lib.mkForce ''
            set -euo pipefail

            credentials_dir="$CREDENTIALS_DIRECTORY"
            private_key="$credentials_dir/renovateGithubAppPrivateKey"
            . "$credentials_dir/renovate.env"
